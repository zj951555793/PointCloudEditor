...
// 差异只保留在这里：
//   - Desktop OpenGL 2.1+：VAO + VBO；3.2+ 可启用现代 R32UI GPU Picking；
//   - OpenGL ES 3.1：原生支持 VBO + VAO；
//   - Desktop GPU Picking 仅使用 OpenGL 3.2+ / GLSL 150 core + R32UI；
//     更老/异常驱动自动使用 CPU Picking；Picking 不上传第二份几何。
class IRenderBackend {
  public:
    struct Buffers {
        ...
        GLuint meshEbo{0};
        // Mesh 高亮只额外保存选中三角形的索引，不复制 Position/Normal/Color。
        GLuint selectedMeshEbo{0};
    };
    ...
    virtual void bindMeshSelectionLayout(QOpenGLExtraFunctions& gl, const Buffers& b) const = 0;
