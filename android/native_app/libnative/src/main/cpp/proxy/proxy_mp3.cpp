...
void lameInit(JNIEnv* env, jobject instance, jint inSampleRate, jint outChannel, jint outSampleRate, jint outBitRate,
              jint quality) {
    lameInitInternal(inSampleRate, outChannel, outSampleRate, outBitRate, quality);
}

int lameEncode(JNIEnv* env, jobject instance, jshortArray leftBuf_, jshortArray rightBuf_, jint sampleRate,
               jbyteArray mp3Buf_) {
    jint ret = -1;
