...
xtern const int nr_of_sfb_block[6][3][4];
...
nt on_pe(lame_internal_flags* gfc, const FLOAT pe[2][2], int targ_bits[2], int mean_bits, int gr, int cbr);

void reduce_side(int targ_bits[2], FLOAT ms_ener_ratio, int mean_bits, int max_bits);

oid iteration_init(lame_internal_flags* gfc);

nt calc_xmin(lame_internal_flags const* gfc, III_psy_ratio const* const ratio, gr_info* const cod_info,
              FLOAT* l3_xmin);

int calc_noise(const gr_info* const cod_info, const FLOAT* l3_xmin, FLOAT* distort, calc_noise_result* const res,
...
int count_bits(lame_internal_flags const* const gfc, const FLOAT* const xr, gr_info* const cod_info,
               calc_noise_data* prev_noise);
int noquant_count_bits(lame_internal_flags const* const gfc, gr_info* const cod_info, calc_noise_data* prev_noise);

oid best_huffman_divide(const lame_internal_flags* const gfc, gr_info* const cod_info);
