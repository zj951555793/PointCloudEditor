...
CaptureBasePipeline::CaptureBasePipeline(const size_t& _data_bytes)
     AbstractBufferedPipeline(MAX_FRAME_NUM, INIT_FRAME_POOL_SZ, _data_bytes), mIsCapturing(false), captureQueue(NULL),
...
CaptureBasePipeline::CaptureBasePipeline(const int& _max_buffer_num, const int& init_pool_num,
                                         const size_t& default_frame_size)
     AbstractBufferedPipeline(_max_buffer_num, init_pool_num, default_frame_size), mIsCapturing(false),
      captureQueue(NULL), frameWidth(0), frameHeight(0) 
    ENTER();

    EXIT();
}

aptureBasePipeline::~CaptureBasePipeline() {
    ENTER();
...
//
//********************************************************************************
const bool CaptureBasePipeline::isCapturing() const {
    return mIsCapturing;
}
