...
SQLiteBufferedPipeline::SQLiteBufferedPipeline(const char* database_name, const bool& clear_table)
     IPipeline(0), db(NULL), sql_insert_one(NULL), sql_query_oldest_10(NULL), sql_delete_one(NULL),
      sql_delete_older(NULL), sql_count(NULL) 
    ENTER();

    db = new sqlite3pp::database(database_name);
    // 0:id, 1:dtime, 2:format, 3:width, 4:height, 5:sequence, 6:data_bytes, 7:data
    sqlite3pp::command cmd(*db, "CREATE TABLE IF NOT EXISTS " TABLE_NAME " ("
    ...
    // sqlite3pp::statement and it's descendants that accessing table can only create when the specific table already
    // exists.
    sql_insert_one =
        new sqlite3pp::command(*db, "INSERT INTO " TABLE_NAME " (" INSERT_FIELDS ") VALUES (?, ?, ?, ?, ?, ?, ?);");
    sql_query_oldest_10 =
        new sqlite3pp::query(*db, "SELECT " ALL_FIELDS " FROM " TABLE_NAME " ORDER BY dtime LIMIT 10;");
...
void SQLiteBufferedPipeline::do_loop() {
    ...
    ts.tv_sec = 5;
    ts.tv_nsec = 0; // wait maximum 5 sec
                    ...
                    for (auto iter = sql_query_oldest_10->begin(); iter != sql_query_oldest_10->end(); ++iter) {
                        ...
                        if (LIKELY(!uvc_ensure_frame_size(frame, actual_bytes))) {
                            ...
                            if (!chain_frame(frame)) {
                                ...
                                /								LOGI("chain_frame:%lld,%lld,(%d,%d),actual_bytes=%d",
                                //id, dtime, width, height, actual_bytes);
                                if (iter != sql_query_oldest_10->end()) {
                                    // wait several msecs here if the record is not a last one, otherwise after pipeline
                                    // will exceed buffer and drop frame(s)
                                    handler_mutex.lock();
                                    {
                            ...
                            LOGW("uvc_ensure_frame_size failed:%lld,%lld,(%d,%d),actual_bytes=%d", id, dtime, width,
                                 height, actual_bytes);
                        }
                    } // end of for
...
//
//********************************************************************************
static ID_TYPE nativeCreate(JNIEnv* env, jobject thiz, jstring database_path_str, jboolean clear_table) {
...
int register_sqlite_buffered_pipeline(JNIEnv* env) {
    ...
    if (registerNativeMethods(env, "com/serenegiant/usb/SQLiteBufferedPipeline", methods, NUM_ARRAY_ELEMENTS(methods)) <
        0) {
        return -1;
    }
    return 0;
