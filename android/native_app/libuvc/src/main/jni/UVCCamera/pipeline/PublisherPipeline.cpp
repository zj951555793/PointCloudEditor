...
PublisherPipeline::PublisherPipeline(const size_t& _data_bytes, const char* addr, const char* _subscription_id)
     AbstractBufferedPipeline(MAX_FRAME_NUM, INIT_FRAME_POOL_SZ, _data_bytes), host(addr),
      subscription_id(_subscription_id), data_bytes(_data_bytes), context(NULL), publisher(NULL) 
    ENTER();
...
static void build_header(publish_header_t& header, uvc_frame_t* frame) {
    ...
    header.presentation_time_us_le =
        htole64(nsecs_t(frame->capture_time.tv_sec) * 1000000LL + nsecs_t(frame->capture_time.tv_usec));
...
int PublisherPipeline::handle_frame(uvc_frame_t* frame) {
    ...
    *
     ...
     * Nexus7(2013, Android5.1.1) => 11n(2.4GHz) => router => 11n(2.4GHz) => GALAXY S5(Android5.0) : NG, slow,
     * periodically drops frames. Nexus9(Android5.1.1) => 11n(5GHz) => router => 11n(2.4GHz) => GALAXY S5(Android5.0) :
     * OK Nexus9(Android5.1.1) => 11n(5GHz) => router => 11n(2.4GHz) => MemoPad7(Android4.4.2) : NG, sometimes become
     * slow, periodically drops frames. GALAXY S5(Android5.0) => 11n(2.4GHz) => router => 11n(5Ghz) =>
     * Nexus9(Android5.1.1) : OK GALAXY S5(Android5.0) => 11n(2.4GHz) => router => 11n(2.4Ghz) => GALAXY
     * note2(Android4.4.2) : NG, slow, periodically drops frames. saki
     /
...
//
//********************************************************************************
static ID_TYPE nativeCreate(JNIEnv* env, jobject thiz, jstring publisher_addr_str, jstring subscription_id_str) {
...
static void nativeDestroy(JNIEnv* env, jobject thiz, ID_TYPE id_pipeline) {
...
static jint nativeGetState(JNIEnv* env, jobject thiz, ID_TYPE id_pipeline) {
...
static jint nativeSetPipeline(JNIEnv* env, jobject thiz, ID_TYPE id_pipeline, jobject pipeline_obj) {
...
static jint nativeStart(JNIEnv* env, jobject thiz, ID_TYPE id_pipeline) {
...
static jint nativeStop(JNIEnv* env, jobject thiz, ID_TYPE id_pipeline) {
...
int register_publisher_pipeline(JNIEnv* env) {
    ...
    if (registerNativeMethods(env, "com/serenegiant/usb/PublisherPipeline", methods_publisher_pipeline,
                              NUM_ARRAY_ELEMENTS(methods_publisher_pipeline)) < 0) {
        return -1;
    }
    return 0;
}
