...
AbstractBufferedPipeline::AbstractBufferedPipeline(const int& _max_buffer_num, const int& _init_pool_num,
                                                   const size_t& _default_frame_size,
                                                   const bool& drop_frames_when_buffer_empty)
     IPipeline(_default_frame_size), max_buffer_num(_max_buffer_num), init_pool_num(_init_pool_num),
      drop_frames(drop_frames_when_buffer_empty), total_frame_num(0) 
    ENTER();
...
int AbstractBufferedPipeline::add_frame(uvc_frame_t* frame) {
    ...
    buffer_mutex.lock();
    {
        // FIXME as current implementation, transferring frame data on my device is slower than that coming from UVC
        // camera... just drop them now
        if (frame_buffers.size() > max_buffer_num) {
            // erase old frames
            int cnt = 0;
            for (auto iter = frame_buffers.begin(); (iter != frame_buffers.end()) && (cnt < 5); iter++, cnt++) {
                recycle_frame(*iter);
