...
IPipeline::IPipeline(const size_t& _default_frame_size)
     state(PIPELINE_STATE_UNINITIALIZED), mIsRunning(false), default_frame_size(_default_frame_size),
      next_pipeline(NULL) 
    ENTER();
...
/*public*/
const bool IPipeline::isRunning() const {
    return (mIsRunning);
};

/*public*/
const pipeline_state_t IPipeline::getState() const {
    return (state);
};

/*protected*/
void IPipeline::setState(const pipeline_state_t& new_state) {
    state = new_state;
}
