...
PreviewPipeline::PreviewPipeline(const size_t& _data_bytes)
     CaptureBasePipeline(MAX_FRAME_NUM, INIT_FRAME_POOL_SZ, _data_bytes), mCaptureWindow(NULL) 
    ENTER();
                    ...
                    capture_mutex.lock();
                    {
                        ...
                        if (LIKELY(window)) {
                            if (UNLIKELY(need_update_geometry)) {
                                ...
                                ANativeWindow_setBuffersGeometry(window, frameWidth, frameHeight,
...
static jint nativeSetCaptureDisplay(JNIEnv* env, jobject thiz, ID_TYPE id_pipeline, jobject jSurface) {
...
int register_preview_pipeline(JNIEnv* env) {
    ...
    if (registerNativeMethods(env, "com/serenegiant/usb/PreviewPipeline", methods, NUM_ARRAY_ELEMENTS(methods)) < 0) {
        return -1;
    }
    return 0;
}
