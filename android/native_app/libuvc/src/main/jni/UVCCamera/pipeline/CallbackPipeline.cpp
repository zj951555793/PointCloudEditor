...
CallbackPipeline::CallbackPipeline(const size_t& _data_bytes)
     CaptureBasePipeline(MAX_FRAME_NUM, INIT_FRAME_POOL_SZ, _data_bytes), mFrameCallbackFunc(NULL),
...
static jint nativeSetFrameCallback(JNIEnv* env, jobject thiz, ID_TYPE id_pipeline, jobject jIFrameCallback,
                                   jint pixel_format) {
...
int register_callback_pipeline(JNIEnv* env) {
    ...
    if (registerNativeMethods(env, "com/serenegiant/usb/FrameCallbackPipeline", methods, NUM_ARRAY_ELEMENTS(methods)) <
        0) {
        return -1;
    }
    return 0;
