...
UVCButtonCallback::UVCButtonCallback(uvc_device_handle_t* devh)  mDeviceHandle(devh), mButtonCallbackObj(NULL) {
    ...
    pthread_mutex_lock(&button_mutex);
    {
        if (!env->IsSameObject(mButtonCallbackObj, button_callback_obj)) {
            ...
            if (button_callback_obj) {
                ...
                if (LIKELY(clazz)) {
                    ibuttoncallback_fields.onButton = env->GetMethodID(clazz, "onButton", "(II)V");
                } else {
