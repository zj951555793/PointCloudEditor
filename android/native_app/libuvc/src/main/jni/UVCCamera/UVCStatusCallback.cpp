...
UVCStatusCallback::UVCStatusCallback(uvc_device_handle_t* devh)  mDeviceHandle(devh), mStatusCallbackObj(NULL) {
    ...
    pthread_mutex_lock(&status_mutex);
    {
        if (!env->IsSameObject(mStatusCallbackObj, status_callback_obj)) {
            ...
            if (status_callback_obj) {
                ...
                if (LIKELY(clazz)) {
                    istatuscallback_fields.onStatus =
                        env->GetMethodID(clazz, "onStatus", "(IIIILjava/nio/ByteBuffer;)V");
                } else {
...
void UVCStatusCallback::notifyStatusCallback(JNIEnv* env, uvc_status_class status_class, int event, int selector,
    ...
    pthread_mutex_lock(&status_mutex);
    {
        if (mStatusCallbackObj) {
            ...
            env->CallVoidMethod(mStatusCallbackObj, istatuscallback_fields.onStatus, (int)status_class, event, selector,
                                (int)status_attribute, buf);
            env->ExceptionClear();
...
void UVCStatusCallback::uvc_status_callback(uvc_status_class status_class, int event, int selector,
                                            uvc_status_attribute status_attribute, void* data, size_t data_len,
                                            void* user_ptr) {
