#ifndef _RULERMVS_CORE_NANOFLANN_HPP_
#define _RULERMVS_CORE_NANOFLANN_HPP_
#include <algorithm>
#include <array>
#include <cassert>
...
/**
 * Traits if object is resizable and assignable (typically has a resize |
 * assign method)
 */
...
template <typename Container> inline
    typename std::enable_if<!has_resize<Container>::value, void>::type
...
template <typename Container, typename T> inline
    typename std::enable_if<!has_assign<Container>::value, void>::type
...
struct L1_Adaptor {
    ...
    inline DistanceType evalMetric(const T* a, const AccessorType b_idx,
        size_t size, DistanceType worst_dist = -1) const
    {
 ...
 *  Corresponding distance traits: rulermvs::metric_SO2
 *
 * \tparam T Type of the elements (e.g. double, float, uint8_t)
 * \tparam DataSource Source of the data, i.e. where the vectors are stored
 ...
 *  Corresponding distance traits: rulermvs::metric_SO3
 *
 * \tparam T Type of the elements (e.g. double, float, uint8_t)
 * \tparam DataSource Source of the data, i.e. where the vectors are stored
 * \tparam _DistanceType Type of distance variables (must be signed) (e.g.
 ...
 * First, it is far more efficient for allocating small objects, as there is
...
public:
    ...
    void* malloc(const size_t req_size)
    {
         ...
         */
        const Size size =
            static_cast<Size>((req_size + (WORDSIZE - 1)) & ~(WORDSIZE - 1));
        ...
        if (size > remaining) {
            ...
            const Size blocksize = static_cast<Size>(
            ...
            remaining = static_cast<Size>(blocksize - sizeof(void*) - shift);
...
/** Used to declare fixed-size arrays when DIM>0, dynamically-allocated
 * vectors when DIM=-1. Fixed size version for a generic DIM:
 */
...
/** kd-tree base-class
 *
 * Contains the member functions common to the classes
 * KDTreeSingleIndexAdaptor and KDTreeSingleIndexDynamicAdaptor_.
 *
 * \tparam Derived The name of the class which inherits this class.
 * \tparam DatasetAdaptor The user-provided adaptor (see comments above).
 * \tparam Distance The distance metric to use, these are all classes
 * derived from rulermvs::Metric \tparam DIM Dimensionality of data points
 * (e.g. 3 for 3D points) \tparam AccessorType Will be typically size_t or
 * int
 */
...
public:
    ...
    struct Node {
         ...
         * non-leaf node, so both data fields are never used simultaneously
         */
        union {
            struct leaf {
    ...
    void computeMinMax(const Derived& obj, Offset ind, Size count,
        Dimension element, ElementType& min_elem, ElementType& max_elem)
    {
    ...
    void planeSplit(Derived& obj, Offset ind, const Size count,
        Dimension cutfeat, DistanceType& cutval, Offset& lim1, Offset& lim2)
    {
        ...
        for (;;) {
            ...
            while (right && left <= right &&
                ...
                --right;
            if (left > right || !right)
                break;  // "!right" was added to support unsigned Index
                        // types
    ...
    DistanceType computeInitialDistances(const Derived& obj,
        const ElementType* vec, distance_vector_t& dists) const
    {
        assert(vec);
 ...
 * returned in "bb" so it can be avoided to redo it again.
 *   //   Look at bb.size() to find out the expected dimensionality (e.g. 2
 * or 3 for point clouds) template <class BBOX> bool kdtree_get_bbox(BBOX
 * &bb) const
 *   {
 ...
 *
 * \tparam DatasetAdaptor The user-provided adaptor (see comments above).
 * \tparam Distance The distance metric to use: rulermvs::metric_L1,
 * rulermvs::metric_L2, rulermvs::metric_L2_Simple, etc. \tparam DIM
...
public:
    ...
    KDTreeSingleIndexAdaptor(const KDTreeSingleIndexAdaptor<Distance,
        DatasetAdaptor, DIM, AccessorType>&) = delete;
    ...
    using BaseClassRef = typename rulermvs::KDTreeBaseClass<
        rulermvs::KDTreeSingleIndexAdaptor<Distance, DatasetAdaptor, DIM,
            AccessorType>,
        Distance, DatasetAdaptor, DIM, AccessorType>;
     ...
     *
     */
    template <class... Args> KDTreeSingleIndexAdaptor(
...
public:
     ...
     */
    template <typename RESULTSET> bool findNeighbors(RESULTSET& result,
        const ElementType* vec, const SearchParams& searchParams) const
    {
        assert(vec);
    ...
    {
        rulermvs::KNNResultSet<DistanceType, AccessorType> resultSet(
            num_closest);
        resultSet.init(out_indices, out_distances_sq);
        this->findNeighbors(resultSet, query_point, rulermvs::SearchParams());
     ...
     * radiusSearch
     */
    template <class SEARCH_CALLBACK> Size radiusSearchCustomCallback(
...
public:
    ...
    void init_vind()
    {
        ...
        for (Size i = 0; i < BaseClassRef::m_size; i++)
            BaseClassRef::vAcc[i] = (AccessorType)i;
    }
     ...
     * are sufficient
     */
    template <class RESULTSET> bool searchLevel(RESULTSET& result_set,
        const ElementType* vec, const NodePtr node, DistanceType mindistsq,
        distance_vector_t& dists, const float epsError) const
    {
        /* If this is a leaf node, then do check and return. */
        if ((node->child1 == nullptr) && (node->child2 == nullptr)) {
            // count_leaf += (node->lr.right-node->lr.left);  // Removed
            // since was neither used nor returned to the user.
        ...
        if (mindistsq * epsError <= result_set.worstDist()) {
            if (!searchLevel(
                ...
                // the resultset doesn't want to receive any more points,
...
public:
    ...
    using BaseClassRef = typename rulermvs::KDTreeBaseClass<
        rulermvs::KDTreeSingleIndexDynamicAdaptor_<Distance, DatasetAdaptor,
            DIM, AccessorType>,
        Distance, DatasetAdaptor, DIM, AccessorType>;
    ...
    KDTreeSingleIndexDynamicAdaptor_(const Dimension dimensionality,
        const DatasetAdaptor& inputData, std::vector<int>& treeIndex_,
        const KDTreeSingleIndexAdaptorParams& params =
            KDTreeSingleIndexAdaptorParams())
        : dataset(inputData)
        , index_params(params)
...
public:
     ...
     * \tparam RESULTSET Should be any ResultSet<DistanceType>
     */
    template <class RESULTSET> void searchLevel(RESULTSET& result_set,
        const ElementType* vec, const NodePtr node, DistanceType mindistsq,
        distance_vector_t& dists, const float epsError) const
    {
        /* If this is a leaf node, then do check and return. */
 ...
 *
 * class to create multiple static index and merge their results to behave
 * as single dynamic index as proposed in Logarithmic Approach.
 *
 *  Example of usage:
...
public:
    ...
    void addPoints(AccessorType start, AccessorType end)
    {
        ...
        for (AccessorType idx = start; idx <= end; idx++) {
            ...
            for (int i = 0; i < pos; i++) {
                for (int j = 0; j < static_cast<int>(index[i].vAcc.size());
                     j++) {
 ...
 * Eigen Matrix, without duplicating the data storage. You can select
 * whether a row or column in the matrix represents a point in the state
 * space.
 *
 * Example of usage:
 * \code
 ...
 *
 * // Fill out "mat"...
 * using my_kd_tree_t = rulermvs::KDTreeEigenMatrixAdaptor<
 ...
 * \endcode
 *
 *  \tparam DIM If set to >0, it specifies a compile-time fixed
 * dimensionality for the points in the data set, allowing more compiler
 * optimizations. \tparam Distance The distance metric to use:
 * rulermvs::metric_L1, rulermvs::metric_L2, rulermvs::metric_L2_Simple,
 * etc. \tparam row_major If set to true the rows of the matrix are used as
 * the points, if set to false the columns of the matrix are used as the
 * points.
 */
template <class MatrixType, int32_t            DIM = -1,
    class Distance = rulermvs::metric_L2, bool row_major = true>
struct KDTreeEigenMatrixAdaptor {
    ...
    using index_t = KDTreeSingleIndexAdaptor<metric_t, self_t,
        ...
        : m_data_matrix(mat)
    {
        ...
        index = new index_t(dims, *this /* adaptor */,
            rulermvs::KDTreeSingleIndexAdaptorParams(leaf_max_size));
    }
...
public:
    ...
    inline void query(const num_t* query_point, const Size num_closest,
        ...
        const int /* nChecks_IGNORED */ = 10) const
    {
        rulermvs::KNNResultSet<num_t, IndexType> resultSet(num_closest);
        resultSet.init(out_indices, out_distances_sq);
        index->findNeighbors(resultSet, query_point, rulermvs::SearchParams());
    }
    ...
    {
        if (row_major) return m_data_matrix.get().rows();
        else
    ...
    {
        if (row_major) return m_data_matrix.get().coeff(idx, IndexType(dim));
        else
