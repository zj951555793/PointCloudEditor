//#pragma once
#ifndef _RULERMVS_MATCH_FaceScan_HPP_
#define _RULERMVS_MATCH_FaceScan_HPP_
...
struct IFaceScan {
    ...
    using Result    = IFaceScanResult;
    using ResultSet = std::vector<const Result*>;
    using Parameter = IFaceScanPara;
    ...
    typedef std::shared_ptr<IFaceScan> Ptr;
    /// @brief 创建实例
    static inline Ptr create();
    
   
    /// @brief 虚析构函数，占位
    virtual inline ~IFaceScan() {}
    /// @brief 初始化
    /// @param RGBD相机内参
    /// @param para 拼接参数
    /// @return 返回是否初始化成功
    virtual bool init(const CameraP&, const IFaceScanPara& para, bool is_vio) = 0;
    /// @brief 暂停
    virtual void stop() = 0;
    ...
    virtual void optimize_rgbd(ProgressBar progress = 0) = 0;
...
/// @brief 创建RGBD拼接方法类
MVS_EXPORT IFaceScan* createRGBDFusion();
template <> struct ObjectCreator_<IFaceScan> {
...
typename IFaceScan::Ptr IFaceScan::create()
{
