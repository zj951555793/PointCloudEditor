#ifndef _RULERMVS_MATCH_RGBDFUSION_HPP_
#define _RULERMVS_MATCH_RGBDFUSION_HPP_
...
struct IRGBDFusion {
    ...
    using Result    = IRGBDFusionResult;
    using ResultSet = std::vector<const Result*>;
    using Parameter = IRGBDFusionPara;
    using RetFunc   = std::function<void(const Result&)>;
    using InputFunc = std::function<void(RGBImage&, Imagef&)>;

    typedef std::shared_ptr<IRGBDFusion> Ptr;
    /// @brief 创建实例
    static MVS_EXPORT Ptr create(const CameraP&, const IRGBDFusionPara&,
        bool is_vio = false, int threadNum = -1);
    /// @brief 虚析构函数，占位
    virtual inline ~IRGBDFusion() {}
    /// @brief 初始化
    /// @param RGBD相机内参
    /// @param para 拼接参数
    /// @return 返回是否初始化成功
    //virtual bool init(const CameraP&, const IRGBDFusionPara&,bool is_vio = false,int threadNum = -1) = 0;
    /// @brief 暂停
    virtual void stop() = 0;
    /// @brief 重置
...
///// @brief 创建RGBD拼接方法类
//MVS_EXPORT IRGBDFusion* createRGBDFusion();
//template <> struct ObjectCreator_<IRGBDFusion> {
...
////};
//
//typename IRGBDFusion::Ptr IRGBDFusion::create()
//{
...
//    return createObject<IRGBDFusion>();
//}


MVS_EXPORT void merge_points_to_cloud(std::vector<RGBPointCloud*>& _clouds,
...
MVS_EXPORT void optimizeMultiGroupClouds(
...
#endif  // _RULERMVS_CORE_RGBDFUSION_HPP_