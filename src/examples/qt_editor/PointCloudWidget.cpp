...
pceditor::example::OrbitCamera::Quat quatFromBasis(const pceditor::Vec3f& rightIn,
                                                     const pceditor::Vec3f& upIn,
                                                     const pceditor::Vec3f& backIn) {
    using Cam = pceditor::example::OrbitCamera;
    const auto r = pceditor::example::normalize(rightIn);
    const auto u = pceditor::example::normalize(upIn);
    const auto b = pceditor::example::normalize(backIn);
...
bool bakeTexture(const pceditor::ObjAppearanceData& appearance, pceditor::PointCloud& cloud) {
...
#pragma omp parallel for schedule(static) num_threads(pceditor::processing::processingThreadCount())
#endif
...
std::shared_ptr<pceditor::PointCloud> deepCopyPointCloud(const std::shared_ptr<pceditor::PointCloud>& src) {
    if (!src)
        return {};
    return std::make_shared<pceditor::PointCloud>(src->points());
}

std::shared_ptr<pceditor::TriangleMesh> deepCopyTriangleMesh(
    const std::shared_ptr<pceditor::TriangleMesh>& src,
    const std::shared_ptr<pceditor::PointCloud>& preferredVertices) {
    if (!src)
        return {};
    ...
    auto out = std::make_shared<pceditor::TriangleMesh>(vertices, src->indices());
...
#ifdef PCEDITOR_HAS_TEXTURE_MAPPING
std::shared_ptr<pceditor::texture::Result> deepCopyTextureResult(
    const std::shared_ptr<pceditor::texture::Result>& src) {
    if (!src)
        return {};
    auto out = std::make_shared<pceditor::texture::Result>(*src);
...
ExportSnapshot makeExportSnapshot(
    const std::shared_ptr<pceditor::PointCloud>& sceneCloud,
    const std::shared_ptr<pceditor::TriangleMesh>& sceneMesh,
    bool meshMode
#ifdef PCEDITOR_HAS_TEXTURE_MAPPING
    , const std::shared_ptr<pceditor::texture::Result>& sceneTextureResult
#endif
) {
    ExportSnapshot out;
...
pceditor::Vec3f smallestEigenVector3x3(float a00, float a01, float a02, float a11, float a12, float a22) {
    ...
    pceditor::Vec3f n{v[0][k],v[1][k],v[2][k]};
    const float l=std::sqrt(n.x*n.x+n.y*n.y+n.z*n.z);
    if(l<1e-12f) return {0,1,0};
    return {n.x/l,n.y/l,n.z/l};
}

bool fitPlanePca(const std::vector<pceditor::Vec3f>& pts, pceditor::Vec3f& center, pceditor::Vec3f& normal) {
...
float dot3(const pceditor::Vec3f&a,const pceditor::Vec3f&b){return a.x*b.x+a.y*b.y+a.z*b.z;}
pceditor::Vec3f sub3(const pceditor::Vec3f&a,const pceditor::Vec3f&b){return {a.x-b.x,a.y-b.y,a.z-b.z};}
pceditor::Vec3f add3(const pceditor::Vec3f&a,const pceditor::Vec3f&b){return {a.x+b.x,a.y+b.y,a.z+b.z};}
pceditor::Vec3f mul3(const pceditor::Vec3f&a,float s){return {a.x*s,a.y*s,a.z*s};}
pceditor::Vec3f cross3(const pceditor::Vec3f&a,const pceditor::Vec3f&b){return {a.y*b.z-a.z*b.y,a.z*b.x-a.x*b.z,a.x*b.y-a.y*b.x};}
pceditor::Vec3f norm3(const pceditor::Vec3f&a){const float l=std::sqrt(dot3(a,a));return l>1e-12f?mul3(a,1.0f/l):pceditor::Vec3f{0,1,0};}
...
ExportSnapshot makeRenderedExportSnapshot(const ExportSnapshot& raw) {
    ...
    if (!raw.meshMode || !raw.mesh) {
        pceditor::PointCloud::Container pts;
        pts.reserve(raw.cloud->activeCount());
        for (const auto& p : raw.cloud->points()) if ((p.flags & pceditor::PointDeleted)==0) {
            auto q=p; q.flags = static_cast<std::uint8_t>(q.flags & ~pceditor::PointSelected); pts.push_back(q);
        }
        out.cloud=std::make_shared<pceditor::PointCloud>(std::move(pts));
        return out;
    }
    const auto& oldPts=raw.cloud->points();
    const auto& idx=raw.mesh->indices();
    std::vector<pceditor::PointId> map(oldPts.size(), pceditor::kInvalidPointId);
    pceditor::PointCloud::Container pts;
    std::vector<std::uint32_t> newIdx;
    newIdx.reserve(raw.mesh->activeTriangleCount()*3u);
    for(std::size_t t=0;t+2<idx.size();t+=3){
        const auto tid=static_cast<pceditor::TriangleId>(t/3u);
        if(!raw.mesh->triangleActive(tid)) continue;
        const std::uint32_t ids[3]{idx[t],idx[t+1],idx[t+2]};
        bool ok=true; for(auto id:ids) if(id>=oldPts.size() || (oldPts[id].flags&pceditor::PointDeleted)) ok=false;
        if(!ok) continue;
        for(auto id:ids){
            if(map[id]==pceditor::kInvalidPointId){auto q=oldPts[id];q.flags=static_cast<std::uint8_t>(q.flags&~pceditor::PointSelected);map[id]=static_cast<pceditor::PointId>(pts.size());pts.push_back(q);} 
            newIdx.push_back(map[id]);
        }
    }
    out.cloud=std::make_shared<pceditor::PointCloud>(std::move(pts));
    out.mesh=std::make_shared<pceditor::TriangleMesh>(out.cloud,std::move(newIdx));
    return out;
}
...
bool projectPointToScreen(const pceditor::Vec3f& position, const pceditor::Mat4f& mvp, int viewportWidth,
...
float adaptiveDepthTolerance(const std::vector<float>& depthPixels, int width, int height, int x, int y,
    ...
    // 不使用固定世界单位/固定 NDC 阈值。用局部 3x3 深度变化估计当前表面的屏幕空间坡度，
    // 再叠加 float/depth-buffer 数值精度余量。平面区域容差自动趋近 0，斜面会随局部坡度增大。
    float diffs[8]{};
    int count = 0;
    for (int dy = -1; dy <= 1; ++dy) {
        ...
        for (int dx = -1; dx <= 1; ++dx) {
            ...
            if (d >= 0.0f && d < 1.0f)
                diffs[count++] = std::abs(d - frontDepth);
        }
    }

    float localSlope = 0.0f;
    if (count > 0) {
        std::sort(diffs, diffs + count);
        // 中位数比 max 更不容易在物体轮廓处被另一层深度污染。
        localSlope = diffs[count / 2];
    }

    const float numerical = std::max(8.0f * std::numeric_limits<float>::epsilon() * std::max(1.0f, std::abs(frontDepth)),
                                     2.0f / 16777215.0f); // 兼容常见 24-bit depth
    return numerical + localSlope * 1.75f;
}

std::vector<pceditor::PointId> selectCandidatePointsAgainstGpuDepth(
    const pceditor::PointCloud& cloud, const std::vector<pceditor::PointId>& candidates, const pceditor::Mat4f& mvp,
    ...
    std::vector<pceditor::PointId> selected;
    ...
    auto depthVisible = [&](float sx, float sy, float depth) {
        ...
        if (lx < 0 || lx >= readWidth || ly < 0 || ly >= readHeight)
            return false;
        const float frontDepth = depthPixels[std::size_t(ly) * std::size_t(readWidth) + std::size_t(lx)];
        if (!(frontDepth >= 0.0f && frontDepth < 1.0f))
            return false;
        const float tolerance = adaptiveDepthTolerance(depthPixels, readWidth, readHeight, lx, ly, frontDepth);
        return depth <= frontDepth + tolerance;
    };

#ifdef PCEDITOR_USE_OPENMP
    const int threadCount = pceditor::processing::processingThreadCount();
    std::vector<std::vector<pceditor::PointId>> perThread(static_cast<std::size_t>(threadCount));
...
#pragma omp for schedule(static)
        for (std::int64_t ii = 0; ii < static_cast<std::int64_t>(candidates.size()); ++ii) {
            ...
            if ((point.flags & pceditor::PointDeleted) != 0)
                continue;
...
std::vector<pceditor::PointId> selectCandidatePointsCpuSurface(
    const pceditor::PointCloud& cloud, const std::vector<pceditor::PointId>& candidates, const pceditor::Mat4f& mvp,
    ...
    for (const auto id : candidates) {
        ...
        const auto& point = cloud.points()[static_cast<std::size_t>(id)];
        if ((point.flags & pceditor::PointDeleted) != 0) continue;
        float sx=0.0f, sy=0.0f, depth=1.0f;
    ...
    auto visible = [&](float sx,float sy,float depth) {
        ...
        const float front=zbuf[static_cast<std::size_t>(ly)*static_cast<std::size_t>(rw)+static_cast<std::size_t>(lx)];
        if (!(front >= 0.0f && front < 1.0f)) return false;
        return depth <= front + adaptiveDepthTolerance(zbuf, rw, rh, lx, ly, front);
    };
#ifdef PCEDITOR_USE_OPENMP
    const int threadCount=pceditor::processing::processingThreadCount();
    std::vector<std::vector<pceditor::PointId>> perThread(static_cast<std::size_t>(threadCount));
...
std::vector<pceditor::PointId> selectCandidatePointsThrough(
    const pceditor::PointCloud& cloud, const std::vector<pceditor::PointId>& candidates, const pceditor::Mat4f& mvp,
...
PointCloudWidget::Model::Model(QString p, std::shared_ptr<pceditor::PointCloud> c, pceditor::ObjMeshData m,
                               bool meshModeFlag)
    ...
    if (meshMode) {
        this->mesh = std::make_shared<pceditor::TriangleMesh>(cloud, std::move(m.triangleIndices));
...
PointCloudWidget::Model::Model(QString p, std::shared_ptr<pceditor::TriangleMesh> meshValue)
...
void PointCloudWidget::ensurePointPickIndex(Model& model) {
    ...
    pceditor::Vec3f bmin = pts.front().position;
    pceditor::Vec3f bmax = pts.front().position;
    ...
    auto cellOf = [&](const pceditor::Vec3f& v) {
    ...
    for (std::size_t i = 0; i < pts.size(); ++i) {
        ...
        model.pickGridIds[cursors[c]++] = static_cast<pceditor::PointId>(i);
    }
...
std::vector<pceditor::PointId> PointCloudWidget::pointPickCandidates(Model& model, const pceditor::Mat4f& mvp,
    ...
    auto blockIntersects = [&](const Model::PickBlock& b) {
        ...
        for (int i = 0; i < 8; ++i) {
            const pceditor::Vec3f v{(i & 1) ? b.max.x : b.min.x, (i & 2) ? b.max.y : b.min.y,
                                    (i & 4) ? b.max.z : b.min.z};
    ...
    workerPool_.start([self, abs] {
        ...
        if (ext == ".obj" || ext == ".OBJ") {
            ...
            if (ext2 == ".txt" || ext2 == ".TXT")
                cloud = pceditor::ModelIO::loadTxt(file, &msg);
            else if (ext2 == ".asc" || ext2 == ".ASC")
                cloud = pceditor::ModelIO::loadAsc(file, &msg);
            else
...
QString PointCloudWidget::beginScanPreview(std::size_t reservePoints) {
    ...
    scanPreviewPath_ = QDir(QDir::tempPath()).absoluteFilePath(QStringLiteral("pceditor_live_scan.ply"));
    auto cloud = std::make_shared<pceditor::PointCloud>();
    cloud->points().reserve(reservePoints);
    pceditor::ObjMeshData emptyMesh;
...
void PointCloudWidget::appendScanPreview(const std::shared_ptr<std::vector<pceditor::Point>>& points,
    ...
    if (compacted) {
        ...
        model.liveBackCloud = std::make_shared<pceditor::PointCloud>(model.cloud->points());
    ...
    if (count > 0) {
        ...
        if (!scanPreviewViewInitialized_) {
            ...
            pceditor::Vec3f bmin = dst[oldSize].position;
            pceditor::Vec3f bmax = dst[oldSize].position;
            ...
            const pceditor::Vec3f frameCenter{
                (bmin.x + bmax.x) * 0.5f,
                (bmin.y + bmax.y) * 0.5f,
                (bmin.z + bmax.z) * 0.5f
            };
...
void PointCloudWidget::appendScanLocalFrame(int frameId,
                                             const std::shared_ptr<std::vector<pceditor::Point>>& localPoints,
    ...
    if (!scanPreviewViewInitialized_) {
        pceditor::PointCloud::Container world;
        world.reserve(localPoints->size());
        for (const auto& src : *localPoints) {
            auto q = src;
            q.position = pceditor::transformPoint(pose, src.position);
            world.push_back(q);
        }
        if (!world.empty()) {
            pceditor::PointCloud tmp(std::move(world));
            camera_.fit(tmp);
            scanPreviewViewInitialized_ = true;
            activeModelIndex_ = index;
            if (scanCameraPose_ && scanCameraPose_->trackingOk) {
...
void PointCloudWidget::upsertScanStatusLayer(
    ...
    pceditor::PointCloud::Container colored;
    ...
    auto cloud = std::make_shared<pceditor::PointCloud>(std::move(colored));
...
void PointCloudWidget::setCurrentScanFrame(const std::shared_ptr<std::vector<pceditor::Point>>& points,
                                           bool trackingOk) {
    ...
    if (trackingOk) {
        ...
        if (points && !points->empty())
            upsertScanStatusLayer(currentScanFramePath_, QStringLiteral("pceditor_current_scan_frame.ply"), points, kGreen);
        else
    ...
    if (!recoveryScanFrameSource_ && currentScanFrameSource_ && currentScanFrameTrackingOk_) {
        ...
        upsertScanStatusLayer(recoveryScanFramePath_, QStringLiteral("pceditor_recovery_reference_frame.ply"),
...
void PointCloudWidget::updateScanCameraPose(const ScanCameraViewPose& pose) {
    ...
    if (pose.trackingOk) {
        ...
        if (scanCameraFollowEnabled_) {
            const auto forward = pceditor::example::normalize(pose.forward);
            const auto right = pceditor::example::normalize(pose.right);
            const auto up = pceditor::example::normalize(pose.up);
            ...
            camera_.target = pceditor::example::add(pose.position, pceditor::example::mul(forward, lookAhead));
            camera_.distance = behind + lookAhead;
            camera_.orientation = quatFromBasis(right, up, pceditor::example::mul(forward, -1.0f));
...
void PointCloudWidget::updateOptimizedScanPreview(const std::shared_ptr<pceditor::PointCloud>&) {
...
void PointCloudWidget::replaceScanPreview(const std::shared_ptr<pceditor::PointCloud>& cloud) {
    if (!cloud)
        return;
    if (scanPreviewPath_.isEmpty())
...
void PointCloudWidget::initializeGL() {
    ...
    qInfo() << "[PointCloudEditor] backend=" << (backend_ ? backend_->name() : QStringLiteral("null"))
...
void PointCloudWidget::createModelGl(Model& m) {
    ...
    glBufferData(GL_ARRAY_BUFFER, n * sizeof(pceditor::Vec3f), nullptr, GL_DYNAMIC_DRAW);
    ...
    if (m.textureResult && m.textureResult->texcoords.size() == m.cloud->size() && !m.textureResult->texcoords.empty()) {
        ...
        glBufferData(GL_ARRAY_BUFFER,
                     static_cast<GLsizeiptr>(m.textureResult->texcoords.size() * sizeof(pceditor::Vec2f)),
...
void PointCloudWidget::uploadPointRangeNow(Model& m, std::size_t first, std::size_t count) {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, m.gpu.positionVbo);
    glBufferSubData(GL_ARRAY_BUFFER, off * sizeof(pceditor::Vec3f), count * sizeof(pceditor::Vec3f), pos.data());
    glBindBuffer(GL_ARRAY_BUFFER, m.gpu.normalVbo);
    glBufferSubData(GL_ARRAY_BUFFER, off * sizeof(pceditor::Vec3f), count * sizeof(pceditor::Vec3f), normal.data());
    glBindBuffer(GL_ARRAY_BUFFER, m.gpu.colorVbo);
...
void PointCloudWidget::uploadLiveBackIncremental(Model& m, std::size_t& budget) {
    ...
    constexpr std::size_t bytesPerPoint = sizeof(pceditor::Vec3f) * 2u + 6u;
...
void PointCloudWidget::drawModel(Model& m) {
    ...
    const auto lightWorld = pceditor::example::normalize(pceditor::example::sub(eye, camera_.target));
    const auto& mm = m.modelTransform.m;
    pceditor::Vec3f lightLocal{
    ...
    lightLocal = pceditor::example::normalize(lightLocal);
    ...
    if (pointMode && m.liveFramePoseMode && !m.liveFrames.empty()) {
        ...
        for (const auto& frame : m.liveFrames) {
            ...
            const auto world = pceditor::example::multiply(m.modelTransform, frame.pose);
            const auto frameMvp = pceditor::example::multiply(vp, world);
...
void PointCloudWidget::drawBasePlaneOverlay() {
    ...
    pceditor::Vec3f ref = std::fabs(basePlane_.normal.y)<0.9f?pceditor::Vec3f{0,1,0}:pceditor::Vec3f{1,0,0};
    ...
    std::array<pceditor::Vec3f,4> local{
...
void PointCloudWidget::drawScanCameraOverlay() {
    ...
    const auto up = pceditor::example::normalize(pose.up);
    const auto forward = pceditor::example::normalize(pose.forward);
    if (pceditor::example::length(forward) < 1.0e-6f)
        return;
    ...
    const auto center = pceditor::example::add(pose.position, pceditor::example::mul(forward, depth));
    const auto rw = pceditor::example::mul(right, halfW);
    const auto uh = pceditor::example::mul(up, halfH);
    std::array<pceditor::Vec3f, 5> v{
        pose.position,
        pceditor::example::add(pceditor::example::add(center, rw), uh),
        pceditor::example::add(pceditor::example::sub(center, rw), uh),
        pceditor::example::sub(pceditor::example::sub(center, rw), uh),
        pceditor::example::sub(pceditor::example::add(center, rw), uh)};
    ...
    const auto axisEnd = pceditor::example::add(pose.position, pceditor::example::mul(forward, depth * 1.8f));
...
void PointCloudWidget::measureActiveSurfaceArea() {
    ...
    const auto r = pceditor::measureSurfaceArea(*model->mesh);
    if (!r.valid) {
...
void PointCloudWidget::measureActiveVolume() {
    ...
    const auto r = pceditor::measureVolume(*model->mesh);
    if (!r.valid) {
...
void PointCloudWidget::startOrRunAutoAlignment() {
    ...
    workerPool_.start([self, sourcePath, targetPath, sourceCloud, targetCloud, sourceTf, targetTf] {
        pceditor::AutoAlignmentOptions opt;
        const auto result = pceditor::alignPointClouds(*sourceCloud, sourceTf, *targetCloud, targetTf, opt);
        QMetaObject::invokeMethod(self, [self, sourcePath, targetPath, result] {
            ...
            if (si < 0 || ti < 0) {
                ...
                case pceditor::AutoAlignmentStatus::NotEnoughPoints: reason = QString::fromUtf8("有效采样点不足"); break;
                case pceditor::AutoAlignmentStatus::NoCorrespondence: reason = QString::fromUtf8("没有稳定对应关系"); break;
                case pceditor::AutoAlignmentStatus::NotConverged: reason = QString::fromUtf8("ICP 未收敛"); break;
                case pceditor::AutoAlignmentStatus::QualityRejected: reason = QString::fromUtf8("质量门槛未通过"); break;
                ...
                sm.modelTransform = pceditor::example::multiply(result.transform, sm.modelTransform);
...
std::optional<pceditor::Vec3f> PointCloudWidget::pickActiveWorldPoint(const QPoint& pos) {
    ...
    pceditor::PointId bestId = pceditor::kInvalidPointId;
    for (const auto id : candidates) {
        const auto* point = model->cloud->tryGet(id);
        if (!point || (point->flags & pceditor::PointDeleted))
            continue;
    ...
    if (bestId == pceditor::kInvalidPointId)
        return std::nullopt;
    const auto* point=model->cloud->tryGet(bestId);
    return pceditor::transformPoint(model->modelTransform, point->position);
}
...
void PointCloudWidget::handleUtilityClick(const QPoint& pos) {
    ...
    if (utilityMode_ == UtilityMode::MeasureDistance) {
        ...
        if (utilityPoints_.size() == 1) {
            ...
            const auto r = pceditor::measureDistance(utilityPoints_[0], utilityPoints_[1]);
    ...
    if (utilityMode_ == UtilityMode::MeasureAngle) {
        ...
        if (utilityPoints_.size() == 1)
            ...
            const auto r = pceditor::measureAngle(utilityPoints_[0], utilityPoints_[1], utilityPoints_[2]);
    ...
    std::array<pceditor::Vec3f,3> target{utilityPoints_[0],utilityPoints_[1],utilityPoints_[2]};
    const auto result = pceditor::alignThreePoints(alignmentSourceWorld_, target);
    if (!result.success) {
    ...
    sourceModel.modelTransform = pceditor::example::multiply(result.transform, sourceModel.modelTransform);
...
pceditor::Mat4f PointCloudWidget::modelMvp(const Model& model) const {
    return pceditor::example::multiply(currentMvp(), model.modelTransform);
}
...
void PointCloudWidget::moveActiveModelByPixels(float dxPixels, float dyPixels) {
    ...
    const auto delta = pceditor::example::add(pceditor::example::mul(right, dxPixels * worldPerPixel),
                                              pceditor::example::mul(up, -dyPixels * worldPerPixel));
...
void PointCloudWidget::fitView() {
    if (auto* model = activeModel(); model && model->cloud) {
        camera_.fit(*model->cloud);
        camera_.target = pceditor::transformPoint(model->modelTransform, camera_.target);
    }
    update();
}
void PointCloudWidget::updateOrbitPivotForActiveModel() {
    ...
    bool valid=false;
    pceditor::Vec3f mn{},mx{};
    auto consume=[&](const pceditor::Point& p){
        if(p.flags&pceditor::PointDeleted) return;
        const auto w=pceditor::transformPoint(model->modelTransform,p.position);
    ...
    if(model->meshMode && model->mesh){
        ...
        for(std::size_t i=0;i+2<idx.size();i+=3){
            if(!model->mesh->triangleActive(static_cast<pceditor::TriangleId>(i/3u))) continue;
...
bool PointCloudWidget::fitBasePlaneFromSelection(QString* message) {
    ...
    if(m->meshMode && m->mesh && !m->selectedTriangleIds.empty()){
        ...
        for(auto id:ids) if(id<m->cloud->size() && !(m->cloud->points()[id].flags&pceditor::PointDeleted)) pts.push_back(m->cloud->points()[id].position);
    } else {
        pts.reserve(m->selectedIds.size());
        for(auto id:m->selectedIds) if(id<m->cloud->size() && !(m->cloud->points()[id].flags&pceditor::PointDeleted)) pts.push_back(m->cloud->points()[id].position);
    }
    ...
    pceditor::Vec3f modelCenter=c; bool bv=false; pceditor::Vec3f mn{},mx{};
    for(const auto&p:m->cloud->points()) if(!(p.flags&pceditor::PointDeleted)) {if(!bv){mn=mx=p.position;bv=true;}else{mn.x=std::min(mn.x,p.position.x);mn.y=std::min(mn.y,p.position.y);mn.z=std::min(mn.z,p.position.z);mx.x=std::max(mx.x,p.position.x);mx.y=std::max(mx.y,p.position.y);mx.z=std::max(mx.z,p.position.z);}}
...
bool PointCloudWidget::applyBasePlaneCut(QString* message) {
    ...
    if(m->meshMode && m->mesh){
        std::vector<pceditor::TriangleId> del; const auto& idx=m->mesh->indices();
        del.reserve(m->mesh->triangleCount()/8u);
        for(std::size_t i=0;i+2<idx.size();i+=3){const auto tid=static_cast<pceditor::TriangleId>(i/3u);if(!m->mesh->triangleActive(tid))continue;const auto ia=idx[i],ib=idx[i+1],ic=idx[i+2];if(ia>=m->cloud->size()||ib>=m->cloud->size()||ic>=m->cloud->size())continue;const auto&a=m->cloud->points()[ia].position;const auto&b=m->cloud->points()[ib].position;const auto&c=m->cloud->points()[ic].position;const pceditor::Vec3f center{(a.x+b.x+c.x)/3.0f,(a.y+b.y+c.y)/3.0f,(a.z+b.z+c.z)/3.0f};if(dot3(sub3(center,planePoint),basePlane_.normal)<0.0f)del.push_back(tid);}
        ...
        std::vector<pceditor::PointId> del;del.reserve(m->cloud->size()/8u);for(pceditor::PointId id=0;std::size_t(id)<m->cloud->size();++id){const auto&p=m->cloud->points()[id];if((p.flags&pceditor::PointDeleted)==0 && dot3(sub3(p.position,planePoint),basePlane_.normal)<0.0f)del.push_back(id);}m->editor.select(del);m->editor.deleteSelection();const auto changed=m->editor.lastChangedIds();makeCurrent();uploadChangedFlags(*m,changed);doneCurrent();statusText_=QString::fromUtf8("已删除基底平面以下 %1 个点").arg(qulonglong(del.size()));
    }
...
void PointCloudWidget::uploadChangedFlags(Model& m, const std::vector<pceditor::PointId>& ids) {
    if (!m.glCreated || !m.cloud)
        return;
    auto sorted = ids;
...
void PointCloudWidget::applySelection(Model& m, std::vector<pceditor::PointId> ids, Qt::KeyboardModifiers modifiers) {
    ids = sortedUnique(std::move(ids));
    ids.erase(std::remove_if(ids.begin(), ids.end(),
                             [&](auto id) {
                                 return id >= m.cloud->size() ||
                                        (m.cloud->points()[id].flags & pceditor::PointDeleted) != 0;
                             }),
              ids.end());
...
void PointCloudWidget::applyTriangleSelection(Model& m, std::vector<pceditor::TriangleId> ids,
    ...
    if (modifiers.testFlag(Qt::ShiftModifier)) {
        ...
        std::vector<pceditor::TriangleId> out;
...
std::vector<pceditor::PointId> PointCloudWidget::filterPickedPointIds(const Model& m,
    ...
    for (auto id : raw) {
        if (id != pceditor::kInvalidPointId && id < m.cloud->size())
            out.push_back(static_cast<pceditor::PointId>(id));
    }
...
void PointCloudWidget::performSurfaceSelection(Qt::KeyboardModifiers modifiers) {
    ...
    auto performCpuSurfaceFallback = [&]() {
        const auto mvp = modelMvp(*m);
        const pceditor::Viewport vp{width(), height()};

        if (triangleIds && m->meshMode && m->mesh) {
            std::vector<pceditor::TriangleId> tids;
            if (interactionMode_ == InteractionMode::Rectangle) {
                tids = pceditor::CpuMeshSelector::surfaceRectangle(
                ...
                tids =
                    pceditor::CpuMeshSelector::surfaceCircle(*m->mesh, mvp, vp, {pressPos_.x(), pressPos_.y()}, radius);
            } else {
                std::vector<pceditor::Point2i> path;
                path.reserve(stroke_.size());
                for (const auto& p : stroke_)
                    path.push_back({p.x(), p.y()});
                tids = (interactionMode_ == InteractionMode::Lasso)
                           ? pceditor::CpuMeshSelector::surfaceLasso(*m->mesh, mvp, vp, path)
                           : pceditor::CpuMeshSelector::surfaceBrushStroke(*m->mesh, mvp, vp, path, brushRadiusPixels_);
            }
    ...
    if (triangleIds && m->meshMode && m->mesh) {
        ...
        for (auto id : picked) {
            if (id == pceditor::kInvalidPointId)
                continue;
            if (m->meshFiltered) {
                ...
                tids.push_back(static_cast<pceditor::TriangleId>(id));
            }
        }
        ...
        if (!tids.empty()) {
            const pceditor::Viewport vp{width(), height()};
            std::vector<pceditor::TriangleId> candidates;
            if (interactionMode_ == InteractionMode::Rectangle) {
                candidates = pceditor::CpuMeshSelector::rectangle(
                    *m->mesh, mvp, vp, {pressPos_.x(), pressPos_.y(), currentPos_.x(), currentPos_.y()});
            } else if (interactionMode_ == InteractionMode::Circle) {
                const int radius = int(std::hypot(currentPos_.x() - pressPos_.x(), currentPos_.y() - pressPos_.y()));
                candidates =
                    pceditor::CpuMeshSelector::circle(*m->mesh, mvp, vp, {pressPos_.x(), pressPos_.y()}, radius);
            } else {
                std::vector<pceditor::Point2i> path;
                path.reserve(stroke_.size());
                for (const auto& p : stroke_)
                ...
                candidates = (interactionMode_ == InteractionMode::Lasso)
                                 ? pceditor::CpuMeshSelector::lasso(*m->mesh, mvp, vp, path)
                                 : pceditor::CpuMeshSelector::brushStroke(*m->mesh, mvp, vp, path, brushRadiusPixels_);
            }

            pceditor::MeshSelectionClosureOptions closureOptions;
            closureOptions.maxRings = 16;
            closureOptions.minAdjacentNormalDot = 0.30f;
            tids = pceditor::MeshSelectionClosure::expandSurfaceSelection(*m->mesh, tids, candidates, closureOptions);
        }
        applyTriangleSelection(*m, std::move(tids), modifiers);
    } else {
...
void PointCloudWidget::performThroughSelection(Qt::KeyboardModifiers modifiers) {
    ...
    if (m->meshMode && m->mesh && m->displayMode != DisplayMode::Points) {
        std::vector<pceditor::TriangleId> tids;
        if (interactionMode_ == InteractionMode::Rectangle) {
            tids = pceditor::CpuMeshSelector::rectangle(
                *m->mesh, mvp, vp, {pressPos_.x(), pressPos_.y(), currentPos_.x(), currentPos_.y()});
        } else if (interactionMode_ == InteractionMode::Circle) {
            const int r = int(std::hypot(currentPos_.x() - pressPos_.x(), currentPos_.y() - pressPos_.y()));
            tids = pceditor::CpuMeshSelector::circle(*m->mesh, mvp, vp, {pressPos_.x(), pressPos_.y()}, r);
        } else {
            std::vector<pceditor::Point2i> path;
            path.reserve(stroke_.size());
            for (const auto& p : stroke_)
...
pceditor::processing::OperationDescriptor PointCloudWidget::processingDescriptor(const std::string& operationId) const {
    auto operation = pceditor::processing::createOperation(operationId);
    if (!operation)
        return {};
    pceditor::processing::ProcessInput input;
    if (const auto* model = activeModel()) {
        input.cloud = model->cloud;
        input.mesh = model->mesh;
    }
    return pceditor::processing::estimateOperationDescriptor(*operation, input);
}

pceditor::processing::ModelDiagnostics PointCloudWidget::activeModelDiagnostics() const {
    pceditor::processing::ProcessInput input;
    if (const auto* model = activeModel()) {
        input.cloud = model->cloud;
        input.mesh = model->meshMode ? model->mesh : nullptr;
    }
    return pceditor::processing::analyzeModel(input);
}

pceditor::processing::ProcessingPreflight
PointCloudWidget::processingPreflight(const std::string& operationId,
                                      const pceditor::processing::ParameterMap& params) const {
    auto operation = pceditor::processing::createOperation(operationId);
    if (!operation)
        return {};
    pceditor::processing::ProcessInput input;
    ...
    return pceditor::processing::preflightOperation(*operation, input, params);
}
...
bool PointCloudWidget::analyzeActiveModelAsync(DiagnosticsFinishedCallback finished) {
    ...
    workerPool_.start([self, path, cloud, mesh, finished = std::move(finished)]() mutable {
        ...
        input.cloud = cloud;
        input.mesh = mesh;
        const auto diagnostics = pceditor::processing::analyzeModel(input);
        QMetaObject::invokeMethod(
            self,
...
bool PointCloudWidget::exportActiveModel(const QString& path, QString* message) {
    ...
    if (snapshot.textureResult && QFileInfo(path).suffix().compare(QStringLiteral("obj"), Qt::CaseInsensitive) == 0) {
        const bool ok = pceditor::texture::saveObj(*snapshot.textureResult, path.toStdString(), &msg, snapshot.mesh.get());
    ...
    const bool ok = rendered.cloud && pceditor::ModelIO::save(*rendered.cloud, rendered.meshMode ? rendered.mesh.get() : nullptr,
        ...
        try {
            ...
            if (!snapshot.cloud) {
                ...
                if (snapshot.textureResult && outPath.endsWith(QStringLiteral(".obj"), Qt::CaseInsensitive)) {
                    ...
                    ok = pceditor::texture::saveObj(*snapshot.textureResult, outPath.toStdString(), &msg,
                                                    snapshot.mesh.get());
                } else
#endif
                {
                    ...
                    auto rendered = makeRenderedExportSnapshot(snapshot);
                    ok = rendered.cloud && pceditor::ModelIO::save(*rendered.cloud,
...
bool PointCloudWidget::startTextureMappingAsync(pceditor::texture::Backend backend, ProcessingFinishedCallback finished) {
    auto* m = activeModel();
    ...
    workerPool_.start([self, sourcePath, sourceMesh, frames, sourceTransform, backend, finished = std::move(finished)]() mutable {
        ...
        cfg.backend = backend;
        cfg.quality = pceditor::texture::Quality::OpenMVS;
        ...
        pceditor::texture::TextureMapper mapper;
        auto result = mapper.map(*sourceMesh, *frames, cfg);
        auto resultPtr = std::make_shared<pceditor::texture::Result>(std::move(result));
        ...
        QMetaObject::invokeMethod(self, [self, sourcePath, sourceMesh, sourceTransform, resultPtr, finished]() mutable {
            ...
            auto mesh = std::make_shared<pceditor::TriangleMesh>(resultPtr->vertices, resultPtr->indices);
            ...
            const QString backendName = resultPtr->backendUsed == pceditor::texture::Backend::Cuda
...
bool PointCloudWidget::startScanTextureMappingAsync(pceditor::texture::Backend backend,
    ...
    auto operation = pceditor::processing::createOperation("poisson_octree");
    if (!operation) {
        if (finished)
    ...
    input.cloud = model->cloud;
    input.mesh = model->mesh;
    const auto desc = pceditor::processing::estimateOperationDescriptor(*operation, input);
    pceditor::processing::ParameterMap params;
...
bool PointCloudWidget::startProcessingOperation(const std::string& operationId,
    ...
    if (desc.inputKind == pceditor::processing::ModelKind::TriangleMesh && (!m->meshMode || !m->mesh)) {
        if (finished)
            finished(false, QString::fromUtf8("该算法需要三角网格模型"));
        return false;
    }
    if (desc.inputKind == pceditor::processing::ModelKind::PointCloud && m->meshMode &&
    ...
    processingBusy_ = true;
    editBusy_ = true;
    processingCancel_ = pceditor::processing::CancelToken{};
    ...
    workerPool_.start([self, path, sourceCloud, sourceMesh, processingCloud, processingMesh, operationId, outputPolicy,
        ...
        if (!self)
            return;
        auto op = pceditor::processing::createOperation(operationId);
        if (!op)
            return;
        ...
        auto result = op->run(
            input, params,
            [self, progress](const pceditor::processing::ProgressInfo& info) {
                if (!self || !progress)
                    return;
        ...
        QMetaObject::invokeMethod(
            ...
            [self, path, sourceCloud, sourceMesh, outputPolicy, preferResultVertexColors, result = std::move(result), finished]() mutable {
                ...
                const bool addOnKindChange = outputPolicy == pceditor::processing::OutputPolicy::AddModelOnKindChange &&
