...
std::vector<PointId> selectSurfaceProjected(const PointCloud& cloud, const Mat4f& mvp, const Viewport& viewport,
    ...
    // Surface selection must remove a thin front shell, not only the mathematically closest sample.
    // Dense/far point clouds have many samples under the same rendered point footprint; selecting only d==zmin
    // leaves the next layer behind after Delete. Keep a bounded screen-depth shell while remaining non-through.
    const float requestedTolerance = std::max(1.0e-6f, depthToleranceNdc);

    // 与 GPU Surface 保持一致：可见性在“当前点中心”采样深度，
    ...
    auto isSurfaceVisible = [&](float sx, float sy, float depth) {
        ...
        const float front = minDepth[pi];
        if (!std::isfinite(front))
            return false;
        const float distanceAwareFloor = 0.00035f + 0.00085f * std::clamp(front, 0.0f, 1.0f);
